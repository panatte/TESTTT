...
function downloadRecording(chunks) {
  ...
  if ($('input[name=radio]:checked').val() == 'webm') {
    ...
    type = 'video/mp4';
  } else if ($('input[name=radio]:checked').val() == 'gif') {
    // Convert WebM to GIF
    convertWebMtoGIF(chunks);
  }
}

// Function to convert WebM to GIF using FFmpeg
function convertWebMtoGIF(chunks) {
  // Send the chunks to your server where FFmpeg is installed
  // Use a server-side script to convert the WebM to GIF
  // Once the conversion is done, provide a link for the user to download the GIF
}
