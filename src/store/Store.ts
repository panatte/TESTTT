...
export class Store {
  ...
  //     const blob = new Blob(chunks, { type: "video/gif" });
  ...
  saveCanvasToVideoWithAudio() {
    this.saveCanvasToVideoWithAudiogifMp4();
  }

  saveCanvasToVideoWithAudiogifMp4() {
    ...
    video.play().then(() => {
      ...
      mediaRecorder.onstop = async function (e) {
        ...
        if (mp4) {
          // lets use ffmpeg to convert gif to mp4
          ...
          await ffmpeg.writeFile('video.gif', data);
          await ffmpeg.exec(["-y", "-i", "video.gif", "-c", "copy", "video.mp4"]);
          // await ffmpeg.exec(["-y", "-i", "video.gif", "-c:v", "libx264", "video.mp4"]);
